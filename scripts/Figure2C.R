...
df <- c(70, 75.2, 24.3, 24.4, 4.7, 
        14.609, 37.889, 9.42,  13.55, 1.697,
        335.445, 149.422, 62.705, 43.842, 13.276)
...
rownames(df) <- c('MetTarget top 100 (unseen)', 'Priority index top 100', 'Open Targets top 100', 'DisGeNET', 'GWAS')
...
df <- c(184.1, 36.1, 19.8, 15.7, 4.4, 
        40.562, 13.668, 5.96, 8.014, 1.353, 
        835.774, 95.157, 65.929, 30.9, 14.584)
...
p <- ggplot(comb_df_long, aes(x = OR, y = rank, color = method, label = method)) + 
  ...
  scale_color_manual(values = c('#C63A29', '#CB7631', '#EBAE44', '#4185B0', '#4EACB9'), 
                     breaks = c('MetTarget top 100 (unseen)', 'Priority index top 100', 'Open Targets top 100', 'DisGeNET', 'GWAS')) +
...
png(filename=paste0('Figure2C.png'),
