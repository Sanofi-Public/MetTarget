...
data_df <- as.data.frame(read.xlsx("../data/data.Figure4D.xlsx", sheet = 1, colNames = TRUE))

data_df[, "source"] <- gsub("Meta-analysis", "pooled colon", data_df[, "source"])

data_df[, "source"] <- factor(data_df[, "source"], 
                         levels = c("GSE10191", "GSE10616", "GSE36807", "GSE75214", "pooled colon"))

data_df[, "gene"] <- factor(data_df[, "gene"], levels = genes)

data_df <- data_df %>% mutate(label = 
                                case_when(source == 'pooled colon' ~
                               paste0('P = ', ifelse(gene %in% c('SPHK1', 'S1PR1'), format(P, scientific = T), round(P, 3)), 
                                      '\nadj. P = ', ifelse(gene %in% c('SPHK1', 'S1PR1'), format(FDR, scientific = T), round(FDR, 3)),
...
p <- ggplot(data=data_df, aes(y = source, x = log2FC, color = source)) +
  ...
  scale_y_discrete(breaks = c("GSE10191", "GSE10616", "GSE36807", "GSE75214", "pooled colon"),
                   labels = c("colon\nGSE10191\n(n=19)", "colon\nGSE10616\n(n=26)", "colon\nGSE36807\n(n=21)", "colon\nGSE75214\n(n=83)", "pooled colon\n(n=149)")) +
  ylab("") +
  xlab("\nLog2 Fold Change") +
  xlim(c(-1, x_max)) +
